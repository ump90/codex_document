...
1. 下载并安装 Codex app
   ...
   下载适用于 macOS 或 Windows 的 Codex app。如果你使用的是 Intel 芯片的 Mac，请选择 Intel 构建版本。
...
如果你之前使用过 Codex app、CLI 或 IDE 扩展，会看到过去处理过的项目。

4. 发送第一条消息

   选择项目后，确保选择 **Local**，让 Codex 在你的机器上工作，然后向 Codex 发送第一条消息。

   你可以向 Codex 询问有关该项目或你的电脑的一般问题。以下是一些示例：
...
使用内置 Git worktree 支持，让并行代码变更保持隔离。
